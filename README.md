...
| coordinator | 教皇 | 并行协调器 | 多 Agent 并行执行、结果聚合、置信度评估 |

## Hook 事件

| 事件 | 触发时机 | 脚本 |
|------|----------|------|
| SessionStart | 会话开始 | hooks/session-init.sh |
| SessionEnd | 会话结束 | hooks/session-state.sh |
| SubagentStop | 子代理结束 | hooks/subagent-aggregate.sh |
| PreCompact | 上下文压缩前 | hooks/pre-compact.sh |
| Stop | 主代理停止 | hooks/ralph-loop-stop.sh |
